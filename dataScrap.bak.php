...
function genPlacesLocDataFromCSV($url)
{
    ...
    $mjrCt->load("majorplaces.xml");
    $mjrCt->normalizeDocument();
    $xPath = new DOMXPath($mjrCt);
    //e v2
    $placeLDxml = new DOMDocument('1.0','UTF-8');
    $placeLDxml->preserveWhiteSpace = false;
    $placeLDxml->formatOutput = true;
    $placeLDxml->normalizeDocument();
    $root = $placeLDxml->createElement("places-location-data");
    $placesTree= new DOMElement("places");
    $root->appendChild($placesTree);
    $explodedLine;
    $i=-1;
    foreach ($lines as $line) {
        $placeNode= new DOMElement(PLACE);
        $placesTree->appendChild($placeNode);
        ...
        $placeNode->setAttribute( ALTID,   trim($explodedLine[5]) );//altitude
        //s v2
        //echo $name.PHP_EOL;
        if($name=="" || $name == NULL)  { continue; }
        $name = remove_accents($name);
        $query = "//place[@".NAME."='".$name."']";
        $respQuery = $xPath->query($query);
        $placeNode->setAttribute( "t", $respQuery->length);
        if($country=="Bangladesh"
                ...
                || $name=="Other City")$placeNode->setAttribute( "t", 1);
        //if($respQuery->length > 0) echo $respQuery->item (0)->getAttribute("C");
        //e v2
    }
...
function PlacesCoordCSV2XML($csvfile, $xmlfile)
{
    ...
    $placesRoot = $placesXml->createElement("places");
...
//    $placeLDxml->
    ...
    $placeLDxml->formatOutput = true;
    $placeLDxml->normalizeDocument();
    $root = $placeLDxml->createElement("places-location-list");
    $placesTree= new DOMElement("places");
    $root->appendChild($placesTree);
    $explodedLine;
    $i=0;
    foreach ($lines as $line) {
        ...
        $placeNode->setAttribute( "id",  $i++ );
        $placeNode->setAttribute( CNTRY,   trim($explodedLine[0]) );
        $placeNode->setAttribute( NAME,trim($explodedLine[1]) );
        $placeNode->setAttribute( LATID,   trim($explodedLine[2]) );//latitude
        $placeNode->setAttribute( LONGD,   trim($explodedLine[3]) );//longitude
    }
    $placeLDxml->appendChild($root);
    $placeLDxml->normalizeDocument();
//    echo $placeLDxml->saveXML();
    $placeLDxml->save("majorcities.xml");
}

function genMajorPlacesFromFile($url){
    $lines = file($url);
...
//    $placeLDxml->
    ...
    foreach ($lines as $line) {
        ...
        $placeNode= new DOMElement(PLACE);
        $placesTree->appendChild($placeNode);
        $placeNode->setAttribute( NAME,remove_accents($name) );
        $placeNode->setAttribute( "fullname",   $name );
    }
    $placeLDxml->appendChild($root);
    $placeLDxml->normalizeDocument();
//    echo $placeLDxml->saveXML();
    $placeLDxml->save("majorplaces.xml");
    XMLUtil::cleanXMLDOC("majorplaces.xml");
}

function genWorldTimeZoneArrayFromFile($url) {
    $lines = file($url);
    $endline = "";
    $tzoneData="";
    foreach ($lines as $line) {
        $line = remove_accents(trim(str_replace('"', '', $line)));
        $wtz = explode(',', $line);
        $tzoneData.="['".trim($wtz[0])."','".trim($wtz[1])."','".trim($wtz[2])."',".trim($wtz[3])."],".$endline;
    }
    echo $tzoneData;
}
...
function genArrayFromXML($endline) {
    ...
    $CountryMap = "";
    $CityLocData = "";
    foreach ($placesTree as $place) {
        if ($place->getAttribute("t") >= 0) {
            $i++;
            $country = $place->getAttribute(CNTRY);
            $name = $place->getAttribute(NAME);
            $lattd = $place->getAttribute(LATID);
            ...
            if ($country != $prevContry) {
                if ("" != $prevContry) {
                    $ccEnd = $i-1;
                    $ccCount = $i-$ccStart;
                    $CountryMap.= "['$prevContry',$ccStart,$ccEnd],".$endline;
                    $numberOfCountries++;
                }
                $prevContry = $country;
                $ccStart=$i;
